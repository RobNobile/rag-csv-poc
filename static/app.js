...
function initializeEventListeners() {
  ...
  uploadArea.addEventListener("click", (e) => {
    // Only trigger file input if not clicking on the label (which already triggers it)
...
async function handleSendMessage() {
  ...
  try {
    ...
    if (result.success) {
      ...
      // Check if error is due to uninitialized RAG
      if (result.error === "RAG system not initialized") {
        console.error("Backend reports RAG not initialized");
        isInitialized = false;
        addMessage(
          '⚠️ System lost initialization. Please click "Upload New CSV" to re-upload your file.',
          "system"
        );
        ...
        if (
          confirm(
            "The RAG system needs to be re-initialized. Would you like to upload a CSV file now?"
          )
        ) {
          handleReset();
        }
...
async function handleReset() {
  if (
    !confirm(
      "Are you sure you want to upload a new CSV? This will clear the current chat session."
    )
  ) {
    return;
  }
